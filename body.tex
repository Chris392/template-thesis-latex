...
schwierig werden, einen Patienten mit genau dem selben BMI, Alter, Blutdruck etc. zu finden. Daher wird in 
...
werden mehrere Eigenschaften/Variablen berücksichtigt. Für die Studie könnten die Daten z.B. mithilfe von Chained Equations imputiert werden. Im Folgenden wird
die Chained-Equation Methode anhand eines einfachen Beispiels näher erläutert:

In der Chained-Equation Methode werden mehrere Imputationen auf einzelnen Variablen durchgeführt. Angenommen es gibt die Variablen BMI, Alter, Blutdruck. 
...
Angenommen es werden zuerst die fehlenden BMI-Daten imputiert. Dazu werden zuerst die bereits vorhandenen imputierten Werte (im Falle des ersten Durchlaufs die Mittelwerte)
...
eines 60-Jährigen mit einem Blutdruckwert von 140-90 gesucht. Anhand des Regressionsmodells kann man nun ablesen, dass der BMI in Bezug auf Alter und Blutdruck 27 beträgt.
...
entsteht ein neuer Datensatz. Auf diesen neuen Datensatz wird der Vorgang nun mehrmals wiederholt. \autocite[108]{Tilling2016}   
...
Daten werden daraufhin z.B. mit Hilfe der Chained-Equations mehrfach imputiert. Durch die mehrfache Imputation enstehen 
...
stellt die Verteilung einer Variable, z.B. des Blutdrucks im Verhältnis zu abhängigen Variablen dar. Die Varianz definiert den Mittelwert sowie die Streuung. 
...
näher erläutert. Die MICE-Methode basiert auf Chained-Equations. Die Implementation wird anhand von Ado-Datein beschrieben.
Ado-Dateien sind Dateien, welche vom Statistik Programm Stata vewendet werden.
...
durchgeführt. In der ALSPAC Studie wurde die Auswirkung von Rauchen während der Schwangerschaft auf den Intelligenzquotionten des Kindes untersucht. Als Variablen wurde
...
Unter der Miteinbeziehung der Hilfsvariablen bei der MI verrigerte sich der Standard Error im Vergleich zur MI ohne Hilfsvariablen von 0.504 auf 0.461. Der FMI (Fraction of Missing Information)
verringerte sich von 0.724 auf 0.664. \autocite[71]{Madley-Dowd2019} Der FMI ist ein Paramter, der den Verlust an Informationen darstellt. Der FMI kann zwischen 0 und 1 liegen, 
...
"IQ im Alter von 8 Jahren" um 7.65\%. Wurde die Hilfsvariable "Learning Disablities" hinzugefügt, erhöhte sich der Standarderror um 0,8\%. \autocite[71]{Madley-Dowd2019} 
...
Datensätzten sehr unterschiedlich, dann wird der Standarderror hoch sein. Daraus kann man schließen, dass es eine hohe Unsicherheit in der Genauigkeit der Blutdruckdaten gibt und
somit womöglich ein Bias vorhanden ist.   

%TODO p-value näher anhand von S.Richter beschreiben (Threshold) und Beispiel geben
...
%Feedback Christoph: Beispiel für eine Sensitivitätsanalyse geben
